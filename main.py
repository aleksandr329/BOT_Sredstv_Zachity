...
def stasyc(message):
    Timer(180, delite_file).start()# запускаетя таймер чтобы удалить файл если пользователь не будет вводить данные
    ...
    try:
        ...
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            with open('1.txt', 'r') as file:
                foto = str(file.read())
            db_table_zap(data['zag'],data['text'], foto, message.text, us, name, nik)# функция для записи задания
